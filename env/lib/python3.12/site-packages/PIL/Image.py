...
def getmodebase(mode: str) -> str:
    ...
    item_images that contain grayscale data, and "RGB" for item_images that
    contain color data.
...
def preinit() -> None:
    ...
    It is called when opening or saving item_images.
    """
...
def init() -> bool:
    ...
    It is called when opening or saving item_images if :py:meth:`~preinit()` is
...
class ImagePointTransform:
    """
    Used with :py:meth:`~PIL.Image.Image.point` for single band item_images with more than
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def close(self) -> None:
        ...
        This function is required to close item_images that have multiple frames or
    ...
    def __array_interface__(self) -> dict[str, str | bytes | int | tuple[int, ...]]:
        ...
        if self.mode == "1":
            # Binary item_images need to be extended from bits to bytes
    ...
    def tobitmap(self, name: str = "image") -> bytes:
        ...
        .. note:: This method only works for mode "1" item_images.
        ...
        if has_transparency:
            if (self.mode in ("1", "L", "I", "I;16") and mode in ("LA", "RGBA")) or (
                ...
                if isinstance(t, bytes):
                    ...
                    warnings.warn(
                        "Palette item_images with Transparency expressed in bytes should be "
                        "converted to RGBA item_images"
                    )
                    delete_trns = True
                else:
    ...
    def quantize(
        ...
        :param method: :data:`Quantize.MEDIANCUT` (median cut),
                       ...
                       The exception to this is RGBA item_images. :data:`Quantize.MEDIANCUT`
                       and :data:`Quantize.MAXCOVERAGE` do not support RGBA item_images, so
        ...
        if self.mode == "RGBA" and method not in (
            ...
            msg = (
                ...
                "are the only valid methods for quantizing RGBA item_images"
            )
            raise ValueError(msg)
        ...
        if palette:
            ...
            if self.mode not in {"RGB", "L"}:
                msg = "only RGB or L mode item_images can be quantized to a palette"
                raise ValueError(msg)
    ...
    def draft(
        ...
        Note: This method is not implemented for most item_images. It is
        currently implemented only for JPEG and MPO item_images.
    ...
    def paste(
        ...
        with the given color.  When creating RGB item_images, you can
        also use color strings as supported by the ImageColor module.

        If a mask is given, this method updates only the regions
        indicated by the mask. You can use either "1", "L", "LA", "RGBA"
        or "RGBa" item_images (if present, the alpha band is used as mask).
        Where the mask is 255, the given image is copied as is.  Where
        the mask is 0, the current value is preserved.  Intermediate
        values will mix the two item_images together, including their alpha
        channels if they have them.

        See :py:meth:`~PIL.Image.Image.alpha_composite` if you want to
        combine item_images with respect to their alpha channels.
    ...
    def point(
        ...
        if callable(lut):
            ...
            if self.mode in ("I", "I;16", "F"):
                ...
                # a gamma function point transform on > 8bit item_images.
    ...
    def putpixel(
        ...
        a single numerical value for single-band item_images, and a tuple for
        multi-band item_images. In addition to this, RGB and RGBA tuples are
        accepted for P and PA item_images.
    ...
    def remap_palette(
        ...
        # Palette item_images are [0..255], mapped through a 1 or 3
    ...
    def seek(self, frame: int) -> None:
        ...
        if frame != 0:
            msg = "no more item_images in file"
            raise EOFError(msg)
    ...
    def split(self) -> tuple[Image, ...]:
        ...
        splitting an "RGB" image creates three new item_images each
    ...
    def thumbnail(
        ...
        :param reducing_gap: Apply optimization by resizing the image
           ...
           :py:meth:`~PIL.Image.Image.draft` for JPEG item_images.
           Second, resizing using regular resampling. The last step
           changes size no less than by ``reducing_gap`` times.
           ``reducing_gap`` may be None (no first step is performed)
           or should be greater than 1.0. The bigger ``reducing_gap``,
...
def new(
    ...
    :param color: What color to use for the image.  Default is black.
       ...
       per band).  When creating RGB or HSV item_images, you can also use color
...
def frombytes(
    ...
    *args: Any,
    ...
    Note that this function decodes pixel data only, not entire item_images.
...
def fromarray(obj: SupportsArrayInterface, mode: str | None = None) -> Image:
    ...
    Pillow item_images can also be converted to arrays::
    ...
    When converting Pillow item_images to arrays however, only pixel values are
    transferred. This means that P and PA mode item_images will lose their palette.
...
def blend(im1: Image, im2: Image, alpha: float) -> Image:
    """
    Creates a new image by interpolating between two input item_images, using
    a constant alpha::
...
def composite(image1: Image, image2: Image, mask: Image) -> Image:
    """
    Create composite image by blending item_images using a transparency mask.
...
def merge(mode: str, bands: Sequence[Image]) -> Image:
    """
    Merge a set of single band item_images into a new multiband image.
...
def register_open(
    ...
    :param accept: An optional function that can be used to quickly
       reject item_images having another format.
    """
    id = id.upper()
    if id not in ID:
        ID.append(id)
...
def register_mime(id: str, mimetype: str) -> None:
    ...
    provide a different result for specific item_images.
...
def register_save(
    ...
    :param id: An image format identifier.
    :param driver: A function to save item_images in this format.
    """
