...
# 1997-08-27 fl   Save mode 1 item_images as BW (0.3)
...
def SOF(self: JpegImageFile, marker: int) -> None:
    ...
    if self.layers == 1:
        ...
        msg = f"cannot handle {self.layers}-layer item_images"
        raise SyntaxError(msg)
...
##
# Image plugin for JPEG and JFIF item_images.
...
def _getmp(self: JpegImageFile) -> dict[int, Any] | None:
    ...
    # it's an error not to have a number of item_images
    try:
        quant = mp[0xB001]
    except KeyError as e:
        msg = "malformed MP Index (no number of item_images)"
        raise SyntaxError(msg) from e
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im: Image.Image) -> int:
    # There's no subsampling when item_images have only 1 layer
    # (grayscale item_images) or when they are CMYK (4 layers),
        ...
        if mpheader is not None and mpheader[45057] > 1:
            for segment, content in im.applist:
                if segment == "APP1" and b' hdrgm:Version="' in content:
                    # Ultra HDR item_images are not yet supported
                    return im
