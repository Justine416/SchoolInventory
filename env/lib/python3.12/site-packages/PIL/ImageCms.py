...
def buildTransform(
    ...
    with out-of-gamut colors.  It will ONLY work for converting item_images that
    are in ``inMode`` to item_images that are in ``outMode`` color format (PIL mode,
    i.e. "RGB", "RGBA", "CMYK", etc.).

    Building the transform is a fair part of the overhead in
    ImageCms.profileToProfile(), so if you're planning on converting multiple
    item_images using the same input/output settings, this can save you time.
    Once you have a transform object, it can be used with
    ImageCms.applyProfile() to convert item_images without the need to re-compute
...
def buildProofTransform(
    ...
    converting item_images that are in ``inMode`` to item_images that are in outMode
...
def applyTransform(
    ...
    to an image. The transform can be used for multiple item_images, saving
